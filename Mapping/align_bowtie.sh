...
REF_GEN=$CODE_DIR/results/ref_genome/ref_bt2_index/EquCab3
REFBASENAME="${REF_GEN%.*}"
...
samtools sort --threads 7 $RES/converted/$BASE_NAME.bam -o $RES/sorted/$BASE_NAME.sorted.bam
...
samtools flagstat $RES/sorted/$BASE_NAME.sorted.bam > $RES/stats/$BASE_NAME.stat.txt
