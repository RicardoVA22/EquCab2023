...
def getTableInfo(code, *fields):
        ...
        if len(fields) == 0:
                link = "http://www.ebi.ac.uk/ena/portal/api/filereport?accession={}&result=read_run".format(code)
        else:
                # return specified fields
                st_fields = ",".join(fields) # join args
                link = "http://www.ebi.ac.uk/ena/portal/api/filereport?accession={}&result=read_run&fields={}".format(code,st_fields)
...
def main(argv):
        ...
        ### catch args ##
        job_num = int(os.environ['SGE_TASK_ID'])
        sra_list_loc = argv[1]
